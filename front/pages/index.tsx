...
export default function Home({ experiences, projects }) {
  ...
  const navHandler = () => {
    if (section === "Experience") {
      return experiences.map((el: Experience) => {
        return (
          <Experience
            key={el._id}
            company={el.company}
      ...
      return projects.map((el: Project, index: number) => {
        return (
          <Project
            key={el._id}
            project={el.project}
  ...
  return (
    <>
      ...
      <section className="group/side">
        <ul className="flex flex-wrap max-w-2xl mx-auto card:max-w-full card:m-auto hover-hover:[&>*]:group-hover/side:hover:opacity-50 [&>*]:group-hover/side:hover:opacity-100">
          {navHandler()}
          {section === "Experience" ? (
            <li className="text-white font-semibold customHover hover-hover:hover:text-teal-400 text-xl mx-6 sm:mx-12">
      ...
      {showButton ? (
        <button
          className="fixed text-teal-400 z-10 p-5 rounded-full bg-teal-400/20 bottom-8 right-5 hover:bg-teal-400/70 transition-colors"
          onClick={scrollTop}
        >
          <TopRightArrowIcon scrollToTop />
        </button>
      ) : null}
    </>
  );
